...
app = FastAPI(title="HireStack API", version="1.0.0")
...
def on_startup() -> None:
    ...
    try:
        ...
        if n_users > 0:
                    print(f"[HireStack] Seeded {n_users} default user accounts.")
        n = seed_database(db)
        if n > 0:
                    print(f"[HireStack] Seeded database with {n} sample candidates.")
        ...
        if jobs_empty:
            ...
            if azure_configured():
                try:
                    ...
                    print(f"[HireStack] Imported {n_az} jobs from Azure SQL (Jobs_DW).")
                    imported = True
                except Exception as e:  # network/driver/SQL issue → fall back
                    print(f"[HireStack] Azure import failed ({e!r}); falling back to seed_jobs.")
                    db.rollback()
            if not imported:
                n_seed = seed_jobs_demo(db)
                if n_seed > 0:
                    print(f"[HireStack] Seeded {n_seed} demo jobs from seed_jobs.py.")
                    imported = True
            if not imported:
                jobs_count = seed_jobs_from_csv(db)
                if jobs_count > 0:
                    print(f"[HireStack] Imported {jobs_count} jobs from data_jobs.csv.")
    finally:
        db.close()
...
def create_admin_user(db: Session = Depends(get_db)) -> dict:
    ...
    existing = db.execute(select(User).where(User.email == "admin@hirestack.ai")).scalar_one_or_none()
    if existing:
        return {"message": "Admin already exists.", "email": "admin@hirestack.ai"}
    admin = User(
        full_name="Admin",
        email="admin@hirestack.ai",
        hashed_password=hash_password("Admin1234!"),
        role="admin",
    )
    db.add(admin)
    db.commit()
    return {"message": "Admin created.", "email": "admin@hirestack.ai", "password": "Admin1234!"}
...
def _build_ai_analysis(cv: "CandidateCV | None", required_skills_str: str | None) -> dict:
    ...
    if (cv.hire_probability or 0) >= 0.7:
        strengths.append(f"Strong hiring probability ({match_score:.0f}%)")
    if matched:
        strengths.append(f"Key skills mastered: {', '.join(matched[:3])}")
    if exp >= 3:
        strengths.append(f"{exp:.0f} years of professional experience")
    if cv.certifications:
        strengths.append(f"Certifications: {cv.certifications[:60]}")
    if cv.projects_count and cv.projects_count >= 3:
        strengths.append(f"{cv.projects_count} projects completed")

    red_flags = []
    if (cv.ai_score or 0) < 50:
        red_flags.append(f"Low AI score ({cv.ai_score:.0f}/100)")
    if missing:
        red_flags.append(f"Missing skills: {', '.join(missing[:3])}")
    if exp < 1:
        red_flags.append("Very limited experience declared")
...
DEFAULT_CHATBOT_RESPONSES = {
    "hello": "Hello! I am the HireStack Assistant. Welcome!\n\nHow can I help you?\n\n📋 Resume Analysis\n💼 Job Recommendations\n❓ Questions about HireStack",
    "hi": "Hello! I am the HireStack Assistant. Welcome!\n\nHow can I help you?\n\n📋 Resume Analysis\n💼 Job Recommendations\n❓ Questions about HireStack",
    "bonjour": "Hello! I am the HireStack Assistant. Welcome!\n\nHow can I help you?\n\n📋 Resume Analysis\n💼 Job Recommendations\n❓ Questions about HireStack",
    "analyze": "Our system uses 6 trained ML models:\n\n✅ **Hiring Prediction** - 100% Accuracy\n✅ **Salary Prediction** - MAE: $21,106\n✅ **Role Classification** - 24 categories, F1: 75.5%\n✅ **Job Recommendation** - TF-IDF + Jaccard on 20k+ postings\n\nVisit https://hirestack-frontend.onrender.com to try it!",
    "analyse": "Our system uses 6 trained ML models:\n\n✅ **Hiring Prediction** - 100% Accuracy\n✅ **Salary Prediction** - MAE: $21,106\n✅ **Role Classification** - 24 categories, F1: 75.5%\n✅ **Job Recommendation** - TF-IDF + Jaccard on 20k+ postings\n\nVisit https://hirestack-frontend.onrender.com to try it!",
    "job": "HireStack recommends jobs based on:\n✅ Your extracted skills\n✅ Your professional experience\n✅ Your job category (24 categories)\n✅ Similarity with 20,414 real job postings\n\nTry it now: https://hirestack-frontend.onrender.com",
    "emploi": "HireStack recommends jobs based on:\n✅ Your extracted skills\n✅ Your professional experience\n✅ Your job category (24 categories)\n✅ Similarity with 20,414 real job postings\n\nTry it now: https://hirestack-frontend.onrender.com",
    "features": "Core features of HireStack:\n\n✨ Automated CV Parsing\n🤖 ML Scoring (hiring probability, salary)\n💼 Intelligent Job Matching\n📊 Dashboards (candidates & recruiters)\n🔔 Real-time Notifications\n📈 Power BI Analytics",
    "help": "I am in demo mode. Here is what you can ask me:\n\n• Hello / Hi / Bonjour\n• Analyze / Models / ML / Analyse\n• Job / Job Openings / Emploi\n• Features / Capabilities\n\nFor more help, visit https://hirestack-frontend.onrender.com",
    "aide": "I am in demo mode. Here is what you can ask me:\n\n• Hello / Hi / Bonjour\n• Analyze / Models / ML / Analyse\n• Job / Job Openings / Emploi\n• Features / Capabilities\n\nFor more help, visit https://hirestack-frontend.onrender.com",
    "default": "I am currently in demo mode with predefined responses.\n\nFor full assistance, visit https://hirestack-frontend.onrender.com or contact our support team."
}
