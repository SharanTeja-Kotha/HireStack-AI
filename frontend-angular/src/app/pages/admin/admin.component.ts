    ...
    <div class="container" style="padding-top:40px;padding-bottom:60px;">
      @if (!auth.isAdmin()) {
        <div class="auth-box">
          ...
          <h2 style="font-size:1.4rem;font-weight:800;margin-bottom:6px;">Admin Access</h2>
          <p style="color:#64748b;font-size:.9rem;margin-bottom:20px;">Sign in with an admin account to continue.</p>
          <div style="text-align:left;">
            <div class="form-group"><label>Email</label><input type="email" [(ngModel)]="loginEmail" placeholder="admin@example.com"></div>
            <div class="form-group" style="margin-top:8px;"><label>Password</label><input type="password" [(ngModel)]="loginPwd" placeholder="••••••••" (keyup.enter)="doLogin()"></div>
          </div>
          @if (authError()) { <p style="color:#ef4444;font-size:12px;margin-top:8px;">{{ authError() }}</p> }
          <button class="btn btn-primary" style="width:100%;margin-top:14px;" (click)="doLogin()">Sign In</button>
        </div>
      } @else {
        ...
        <div class="admin-grid">
          @if (stats()) {
            <div class="stat-card"><div class="stat-val" style="color:#0ea5e9;">{{ stats()!.total }}</div><div class="stat-lbl">Total Resumes</div></div>
            <div class="stat-card"><div class="stat-val" style="color:#16a34a;">{{ stats()!.hired_count }}</div><div class="stat-lbl">Recommended</div></div>
            <div class="stat-card"><div class="stat-val" style="color:#7c3aed;">{{ (stats()!.avg_hire_probability*100).toFixed(1) }}%</div><div class="stat-lbl">Avg. Hiring Probability</div></div>
            <div class="stat-card"><div class="stat-val" style="color:#ea580c;">\${{ formatSalary(stats()!.avg_predicted_salary) }}</div><div class="stat-lbl">Avg. Predicted Salary</div></div>
          }
          @if (jobsStats()) {
            <div class="stat-card"><div class="stat-val" style="color:#1d4ed8;">{{ jobsStats()!.total_jobs.toLocaleString() }}</div><div class="stat-lbl">Total Job Postings</div></div>
            <div class="stat-card"><div class="stat-val" style="color:#0891b2;">{{ jobsStats()!.remote_jobs.toLocaleString() }}</div><div class="stat-lbl">Remote Jobs</div></div>
            <div class="stat-card"><div class="stat-val" style="color:#059669;">{{ jobsStats()!.jobs_with_salary.toLocaleString() }}</div><div class="stat-lbl">With Salary</div></div>
          }
          <div class="stat-card"><div class="stat-val" style="color:#7c3aed;">{{ users().length }}</div><div class="stat-lbl">Users</div></div>
          <div class="stat-card"><div class="stat-val" style="color:#0369a1;">{{ companies().length }}</div><div class="stat-lbl">Recruiters</div></div>
        </div>

        <!-- Tabs -->
        <div class="tab-bar">
          <button class="tab-btn" [class.active]="activeTab()==='jobs'" (click)="activeTab.set('jobs')"><i class="fas fa-briefcase"></i> Jobs & Scraping</button>
          <button class="tab-btn" [class.active]="activeTab()==='recruiters'" (click)="activeTab.set('recruiters')"><i class="fas fa-building"></i> Recruiters</button>
          <button class="tab-btn" [class.active]="activeTab()==='users'" (click)="activeTab.set('users')"><i class="fas fa-users-cog"></i> Users</button>
          ...
          <div class="section-card">
            ...
            <div style="display:flex;gap:12px;flex-wrap:wrap;margin-bottom:16px;">
              <button class="action-btn btn-orange" (click)="triggerScrape()" [disabled]="scraping()">
                @if (scraping()) { <i class="fas fa-spinner fa-spin"></i> Scraping... } @else { <i class="fas fa-spider"></i> Launch Scraper }
              </button>
              ...
              <button class="action-btn btn-purple" (click)="reseedAll()" [disabled]="reseeding()">
                @if (reseeding()) { <i class="fas fa-spinner fa-spin"></i> Reseed in progress... } @else { <i class="fas fa-database"></i> Import Seed Data }
              </button>
            </div>
            ...
            @if (jobsStats()) {
              <div style="display:flex;gap:20px;flex-wrap:wrap;margin-top:16px;font-size:.88rem;color:#64748b;">
                <span><i class="fas fa-briefcase"></i> <strong>{{ jobsStats()!.total_jobs.toLocaleString() }}</strong> total jobs</span>
                <span><i class="fas fa-home"></i> <strong>{{ jobsStats()!.remote_jobs.toLocaleString() }}</strong> remote</span>
                <span><i class="fas fa-dollar-sign"></i> <strong>{{ jobsStats()!.jobs_with_salary.toLocaleString() }}</strong> with salary</span>
              </div>
            }
          </div>
        }
        ...
        @if (activeTab() === 'recruiters') {
          <div class="section-card">
            <h3><i class="fas fa-building"></i> Recruiters & Companies</h3>
            @if (loadingCompanies()) {
              <div style="text-align:center;padding:20px;color:#94a3b8;"><i class="fas fa-spinner fa-spin"></i> Loading...</div>
            } @else if (companies().length) {
              <div style="overflow-x:auto;">
                <table>
                  <thead><tr><th>ID</th><th>Name</th><th>Email</th><th>Company</th><th>Jobs</th><th>Status</th><th>Actions</th></tr></thead>
                  <tbody>
                    @for (c of companies(); track c['id']) {
                      <tr>
                        ...
                        <td>{{ c['company'] || '—' }}</td>
                        <td>{{ c['post_count'] }}</td>
                        <td><span class="badge" [class]="c['is_active'] ? 'badge-active' : 'badge-inactive'">{{ c['is_active'] ? 'Active' : 'Inactive' }}</span></td>
                        <td>
                          @if (c['is_active']) {
                            <button class="action-btn btn-red" style="padding:4px 10px;font-size:11px;" (click)="deactivateUser(+c['id']!)">Deactivate</button>
                          } @else {
                            <button class="action-btn btn-green" style="padding:4px 10px;font-size:11px;" (click)="reactivateUser(+c['id']!)">Activate</button>
                          }
                        </td>
                      </tr>
                    }
              ...
              <p style="color:#94a3b8;">No recruiters found.</p>
            }
          </div>
        }
        ...
        @if (activeTab() === 'users') {
          <div class="section-card">
            ...
            <div style="margin-bottom:12px;">
              <input class="search-input" placeholder="Search by email..." [(ngModel)]="userSearch" (input)="filterUsers()">
            </div>
            @if (loadingUsers()) {
              <div style="text-align:center;padding:20px;color:#94a3b8;"><i class="fas fa-spinner fa-spin"></i> Loading...</div>
              ...
              <div style="overflow-x:auto;">
                <table>
                  <thead><tr><th>ID</th><th>Email</th><th>Name</th><th>Role</th><th>Active</th><th>Created</th><th>Change Role</th><th>Actions</th></tr></thead>
                  <tbody>
                    @for (u of filteredUsers(); track u.id) {
                      <tr>
                        ...
                        <td><span class="badge" [class]="u.is_active ? 'badge-active' : 'badge-inactive'">{{ u.is_active ? 'Yes' : 'No' }}</span></td>
                        ...
                        <td>
                          @if (u.is_active) {
                            <button class="action-btn btn-red" style="padding:4px 10px;font-size:11px;" (click)="deactivateUser(u.id)">Deactivate</button>
                          } @else {
                            <button class="action-btn btn-green" style="padding:4px 10px;font-size:11px;" (click)="reactivateUser(u.id)">Activate</button>
                          }
                        </td>
                      </tr>
                    }
              ...
              <p style="font-size:.78rem;color:#94a3b8;margin-top:8px;">{{ filteredUsers().length }} / {{ users().length }} users</p>
            } @else {
        ...
        @if (activeTab() === 'logs') {
          <div class="section-card">
            ...
            <div style="margin-bottom:12px;">
              <button class="action-btn btn-slate" (click)="loadScrapeLogs()"><i class="fas fa-sync-alt"></i> Refresh</button>
            </div>
            @if (scrapeLogs().length) {
              <div style="overflow-x:auto;">
                <table class="scrape-log">
                  <thead><tr><th>ID</th><th>Source</th><th>Jobs Added</th><th>Status</th><th>Error</th><th>Date</th></tr></thead>
                  <tbody>
              ...
              <p style="color:#94a3b8;">No scraping logs found.</p>
            }
          </div>
        }
      }
...
export class AdminComponent implements OnInit {
  ...
  importCsv(): void {
    ...
    this.api.importJobsCsv().subscribe({
      ...
      error: (e: { error?: { detail?: string } }) => { this.importing.set(false); this.importMsg.set('❌ ' + (e.error?.detail ?? 'Error')); this.importMsgClass.set('action-msg err'); },
    });
  }
  ...
  importScraperOutput(): void {
    ...
    this.api.importScraperOutput().subscribe({
      ...
      error: (e: { error?: { detail?: string } }) => { this.importingScraper.set(false); this.importMsg.set('❌ ' + (e.error?.detail ?? 'Error')); this.importMsgClass.set('action-msg err'); },
    });
  }

  reseedAll(): void {
    if (!confirm('Clear ALL data (users, resumes, jobs, applications) and reimport seed data?')) return;
    ...
    this.api.reseedAll().subscribe({
      next: d => {
        ...
        this.toast.show('Seed data successfully imported!', 'success');
        this.loadAll();
      },
      error: (e: { error?: { detail?: string } }) => {
        this.reseeding.set(false);
        this.importMsg.set('❌ ' + (e.error?.detail ?? 'Reseed error'));
        this.importMsgClass.set('action-msg err');
        this.toast.show('Reseed error', 'error');
      },
    });
  }
  ...
  triggerScrape(): void {
    ...
    this.toast.show('Scraper started... This may take a few minutes.', 'info');
    this.api.triggerScrape().subscribe({
      next: d => {
        this.scraping.set(false);
        const status = d['status'] as string;
        if (status === 'success') {
          this.importMsg.set('✅ Scraping finished: ' + (d['jobs_scraped'] ?? 0) + ' jobs imported');
          this.importMsgClass.set('action-msg ok');
          this.toast.show('Scraping completed successfully!', 'success');
        } else {
      ...
      error: (e: { error?: { detail?: string } }) => { this.scraping.set(false); this.importMsg.set('❌ ' + (e.error?.detail ?? 'Scraping error')); this.importMsgClass.set('action-msg err'); this.toast.show('Scraping error', 'error'); },
    });
  }
  ...
  deactivateUser(id: number): void {
    ...
    this.api.deactivateUser(id).subscribe({
      next: () => { this.toast.show('User deactivated', 'info'); this.loadCompanies(); this.loadUsers(); },
  ...
  reactivateUser(id: number): void {
    this.api.activateUser(id).subscribe({
      next: () => { this.toast.show('User activated', 'success'); this.loadCompanies(); this.loadUsers(); },
      error: () => this.toast.show('Error', 'error'),
    });
  }
  ...
  changeRole(userId: number, newRole: string): void {
    this.api.updateUserRole(userId, newRole).subscribe({
      next: () => { this.toast.show('Role updated to ' + newRole, 'success'); this.loadUsers(); this.loadCompanies(); },
      error: (e: { error?: { detail?: string } }) => this.toast.show('❌ ' + (e.error?.detail ?? 'Error'), 'error'),
    });
  }
  ...
  doLogin(): void {
    ...
    this.auth.login(this.loginEmail, this.loginPwd).subscribe({
      next: (d) => {
        if (d.user.role !== 'admin') { this.authError.set("This account is not an admin."); this.auth.logout(); return; }
        this.loadAll();
        this.toast.show('Connected as admin', 'success');
      },
