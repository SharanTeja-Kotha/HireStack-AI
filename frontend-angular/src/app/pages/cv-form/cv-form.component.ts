...
@Component({
  ...
  template: `
    <div class="container" style="padding-top:40px;max-width:900px;">
      <div class="cv-hero">
        <div class="cv-eyebrow"><span class="dot"></span> AI Analysis</div>
        <h1 class="cv-title">Analyze Your <span class="gradient-text">Resume</span><br>and Unlock Your Potential</h1>
        <p class="cv-lead">Hiring probability, estimated salary, profile cluster, and top job recommendations—computed in seconds by our Machine Learning models.</p>
        <div class="cv-trust">
          <span><img src="assets/icons/percent.svg" alt=""> ML Predictions</span>
          <span><img src="assets/icons/lightning.svg" alt=""> PDF Parsing</span>
          <span><img src="assets/icons/layers.svg" alt=""> 5 ML Models</span>
        </div>
      </div>
      ...
      @if (tab() === 'form') {
        <div class="form-card">
          <h3 style="margin:0 0 20px;font-size:1.05rem;font-weight:700;">Candidate Details</h3>
          <div class="form-grid">
            <div class="form-group"><label>Full Name *</label>
              <input [(ngModel)]="form.full_name" placeholder="John Doe" /></div>
            <div class="form-group"><label>Email *</label>
              <input type="email" [(ngModel)]="form.email" placeholder="john@example.com" /></div>
            <div class="form-group"><label>Phone</label>
              <input [(ngModel)]="form.phone" placeholder="+1 555-0100" /></div>
            ...
            <div class="form-group"><label>Target Job Role *</label>
              <input [(ngModel)]="form.job_role" placeholder="Data Scientist, Developer..." /></div>
            <div class="form-group"><label>Years of Experience *</label>
              <input type="number" [(ngModel)]="form.experience_years" min="0" max="40" /></div>
            <div class="form-group"><label>Number of Projects</label>
              <input type="number" [(ngModel)]="form.projects_count" min="0" /></div>
            <div class="form-group full">
              <div class="range-row">
                <label style="margin:0;">AI Score <small style="font-weight:400;text-transform:none;color:#94a3b8;">(self-evaluation)</small></label>
            ...
            <div class="form-group full"><label>Skills <small style="font-weight:400;text-transform:none;color:#94a3b8;">(press Enter to add)</small></label>
          ...
          <button class="btn btn-primary" style="margin-top:20px;min-width:180px;" (click)="submitForm()" [disabled]="submitting()">
            @if (submitting()) { <i class="fas fa-circle-notch fa-spin"></i> Analyzing... }
            @else { <i class="fas fa-rocket"></i> Analyze CV }
          </button>
        </div>
      }
      ...
      @if (tab() === 'pdf') {
        <div class="form-card">
          <h3 style="margin:0 0 16px;font-size:1.05rem;font-weight:700;">Upload Your PDF Resume</h3>
          <div class="upload-zone" [class.drag]="dragging()"
            ...
            @if (pdfFile()) {
              ...
              <div><img src="assets/icons/cv-upload.svg" alt="" width="62" height="62" style="margin:0 auto 12px;filter:drop-shadow(0 10px 16px rgba(14,165,233,.26));">
                <strong>Drag and drop your CV here</strong> or click to browse<br>
                <span style="font-size:.82rem;color:#94a3b8;">PDF only · Max 10 MB</span>
              </div>
            }
          </div>
          ...
          @if (pdfFile()) {
            <button class="btn btn-primary" style="margin-top:16px;min-width:180px;" (click)="submitPdf()" [disabled]="submitting()">
              @if (submitting()) { <i class="fas fa-circle-notch fa-spin"></i> Analyzing... }
              @else { <i class="fas fa-magic"></i> Analyze PDF }
            </button>
          }
        </div>
      }
      ...
      @if (result()) {
        <div class="result-card">
          <h2 style="font-size:1.3rem;font-weight:800;margin:0 0 20px;">
            <i class="fas fa-chart-pie" style="color:#0ea5e9;"></i> Analysis Results
          </h2>
          <div class="res-grid">
          ...
          @if (result()!.recommendation) {
            <div style="background:rgba(14,165,233,.08);border-radius:12px;padding:16px;border-left:4px solid #0ea5e9;margin-bottom:20px;">
              <strong style="font-size:.82rem;color:#0369a1;text-transform:uppercase;">AI Recommendation</strong>
          ...
          @if (jobRecs().length) {
            <h3 style="font-size:1rem;font-weight:700;margin:0 0 14px;">
              <i class="fas fa-star" style="color:#f59e0b;"></i> Top Recommended Jobs
            </h3>
          ...
          @if (loadingRecs()) {
            <div style="text-align:center;color:#94a3b8;padding:12px;"><i class="fas fa-spinner fa-spin"></i> Loading recommendations...</div>
          }

          <div style="margin-top:20px;display:flex;gap:10px;flex-wrap:wrap;">
            <a routerLink="/jobs" class="btn btn-primary"><i class="fas fa-briefcase"></i> View Jobs</a>
            <a routerLink="/candidate" class="btn btn-outline"><i class="fas fa-user"></i> My Space</a>
            <button class="btn btn-outline" (click)="resetForm()"><i class="fas fa-redo"></i> New Analysis</button>
          </div>
        </div>
      }
    </div>
...
export class CvFormComponent {
  ...
  CLUSTER_NAMES: Record<number, string> = {
    0: 'Confirmed Seniors', 1: 'Technical Experts', 2: 'Promising Juniors', 3: 'Versatile Profiles',
  };
  ...
  onDrop(e: DragEvent): void {
    ...
    else this.pdfError.set('Please drop a PDF file.');
  }
  ...
  submitForm(): void {
    ...
    this.api.createCv(this.form).subscribe({
      next: r => { this.result.set(r); this.submitting.set(false); this.loadRecs(r.id!); this.toast.show('✅ Analysis completed!', 'success'); },
      error: (e: { error?: { detail?: string } }) => { this.submitting.set(false); this.formError.set(e.error?.detail ?? 'Error'); },
    });
  }
  ...
  submitPdf(): void {
    ...
    this.api.uploadCvPdf(file).subscribe({
      next: r => { this.result.set(r); this.submitting.set(false); this.loadRecs(r.id!); this.toast.show('✅ PDF analyzed successfully!', 'success'); },
      error: (e: { error?: { detail?: string } }) => { this.submitting.set(false); this.pdfError.set(e.error?.detail ?? 'Error during PDF analysis.'); },
    });
  }
