...
export class AuthService {
  ...
  private base = ((window as any).__env && (window as any).__env.apiUrl) || environment.apiUrl;
  ...
  login(email: string, password: string): Observable<TokenOut> {
    return this.http
      .post<TokenOut>(`${this.base}/auth/login`, { email, password })
  ...
  register(email: string, password: string, full_name: string, role: string, company?: string): Observable<TokenOut> {
    return this.http
      .post<TokenOut>(`${this.base}/auth/register`, { email, password, full_name, role, company })
      .pipe(tap(r => this._store(r)));
  }
